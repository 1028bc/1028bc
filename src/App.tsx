...
// PROTOCOL IMPORTS
import { useTabProtocol } from './hooks/useTabProtocol';
import { BrandMark } from './components/BrandMark';

export type ExtendedAppView = AppView | 'selection' | 'field-ops' | 'help' | 'admin';
...
export default function App() {
  const { currentBrand, user } = useApp();
  ...
  // INITIALIZE TAB PROTOCOL (#7 BOX & DYNAMIC TITLE)
  useTabProtocol();
  ...
  const renderView = () => {
    switch (currentView) {
      ...
      case 'selection': 
        return (
          <div className="w-full min-h-screen flex items-center justify-center p-6 bg-[#050505] font-display">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl w-full">
              <button onClick={() => setCurrentView('admin')} className="group p-10 bg-[#0a0a0a] border border-white/10 rounded-3xl hover:border-amber-400/50 transition-all text-left relative overflow-hidden">
              ...
              <button onClick={() => setCurrentView('field-ops')} className="group p-10 bg-[#0a0a0a] border border-white/10 rounded-3xl hover:border-sky-400/50 transition-all text-left relative overflow-hidden">
  ...
  return (
    <div 
      className={isFieldOps ? "app-container font-display" : "min-h-screen flex flex-col bg-[#050505] text-white font-display"}
      ...
      >
        <div className="flex min-w-0 flex-1 items-center gap-2 sm:gap-3 pointer-events-auto">
          <div ref={promptWrapRef} className="relative flex shrink-0 items-center">
            <button type="button" onClick={() => { setIsPromptMenuOpen(!isPromptMenuOpen); setIsSettingsOpen(false); }} className="rounded-lg p-1.5 text-white/80 transition-colors hover:bg-white/10 hover:text-white">
              <Terminal size={20} strokeWidth={1.75} />
            </button>
            {isPromptMenuOpen && (
              <div className="absolute left-0 top-full z-50 mt-2 w-56 overflow-hidden rounded-2xl border border-white/10 bg-[#050505]/95 shadow-2xl backdrop-blur-2xl">
                {PROMPT_LINKS.map(({ label, view }) => (
                  <button key={view + label} type="button" onClick={() => { setCurrentView(view); setIsPromptMenuOpen(false); }} className="block w-full border-b border-white/5 px-4 py-3.5 text-left text-[10px] font-black tracking-[0.2em] text-white/90 transition-colors last:border-b-0 hover:bg-white/10 hover:text-white uppercase">
                    {label}
                  </button>
                ))}
              </div>
          ...
          <div className="flex min-w-0 items-center gap-1">
            <span className="truncate font-display text-sm font-normal tracking-tight text-white/40 uppercase">SolUrbana</span>
          </div>
        </div>
        ...
        <div className="flex shrink-0 items-center gap-2 sm:gap-3 pointer-events-auto">
          ...
          <div ref={settingsWrapRef} className="relative">
            ...
            {isSettingsOpen && (
              <div className="absolute right-0 top-full mt-2 w-48 rounded-xl border border-white/10 bg-[#050505]/95 p-2 shadow-2xl backdrop-blur-xl">
                 {isAuthenticated && (
